...
* Ivan Wong - Reflection
  * Things that I learned about software engineering from the project are about time management, importance of planning/designing, and importance of communication between team members. With a project this large during a school term, learning to manage my time wisely and work effectively is a skill that I believe I became better at from the outcome of the project. The next lesson I learned was about the importance of planning and designing software. 80% of the work done by the project is done by prep work with the other 20% focused on implementation. The project seamed daunting at first, but as we designed sketches, described software architecture, design, and interfaces, the project seamed less and less difficult to implement. The last lesson is about communication. Talking with team members and being explicit and obvious is important, so that there's no room for ambiguity between whats expected and what is assumed.
* Tristan Vosburg - Reflection
  * Communication: In the past, I have had many great opportunities to work as a team on projects, however in these environments communication networks, design flow, and delegation of tasks were either, managed by mentors, not necessary, or already set structures that didn't need any changing.  In this team I learned about the tension and consequences that can come from not having these things in place.
  * Web Development Skills (React): In my cs290 class, I took it in-person so React was not covered.  Additionally, web development hasn't been a topic that has "clicked" with me in the past.  When I heard that my team wanted to use React I used this as a chance to try to learn more about it.  However, this has been quite the plunge into the deepend and I don't feel like I learned as much as I could have.
  * Auth0 User Authentication: I was tasked with authorizing and authenticating our users.  This is something that in previous classes we have been warned against, so I was hesitant to go this path.  However, after over a week of trial and error, testing in other GitHub repos, and digging through documentation, I was able to get a proof of concept working in our project.
...
* Kai Lindskog-Coffin - Reflection
  * This project, over the course Software Engineering II, has shown me just how vital planning and testing can be for creating and maintaining a software product. Using testing suites to automate bug discovery, code fragments, repeated code structures, etc., was one of the most eye-opening experiences. These tests provided our team the data neccessary to delegate tasks to fixing errors in the code. Communication has been crucial in completing this project; Since the assignments were released with numbered and bulleted deliverables with clear sections, we were able to divide up the work. A vast majority of the code base was planned prior to its creation, however on the fly changes were directed by design and testing oriented. I have taken many things away from this project, especially when it comes to the processes developers use to automate and streamline production, but I feel the most important was how to work with team members who have a wide variety of skills. Understanding that we all bring a unique skillset that can benefit the project, while using our collective knowledge to document and plan properly.
* Jordan Cowan - Reflection
  * The biggest takeaway for this term was communication. This is something that always needs to be one of the most important things in group work as it makes everything much easier throughout the term. It also led to our best moments of the term as when everyone knew what was going on with the project and what they needed to do then a lot of good progress was made. Next was making sure to make meaningful progress not focusing on every small detail trying to make every little thing perfect but instead working on the important tasks and worrying about everything else later. Finally I learned how much importance needed to be placed on planning. Good planning led to well structured code that was written much quicker and better than code with poor planning. When there was poor planning there was long tedious hours of work that was overall just progress loops with no forward progress being made most of that time which led to poorly structured code and at times redundant sections.
* Raymond Cen
  * I learned that communication is key to an efficient workflow. As a team we were communicating but as the term went on communication slowly died out and we only communicated when deadlines were approaching. I noticed we all worked better when we were all talking to each other and understanding what needed to be done and when it needed it to be done by. Another lesson I learned through this was that implementing and executing the project in a scalable way was very important. Throughout the project, implementation of the web app was very rough since there had been lots of refactoring of the code to make it more scalable in the future. If I could do this project again I would've started out differently by creating a more flexible structure so we didn't spend time on refactoring already implemented code. Lastly, the third lesson I learned was to always plan ahead. During the project there were many moments where I wished I had done something earlier in the project or implemented something a different way. Most of those moments could've been avoided had I planned ahead better. If I were to do this again I would definitely take more time planning on how I wanted this project to go.
